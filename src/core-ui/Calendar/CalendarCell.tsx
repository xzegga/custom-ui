...
import { useCalendarCell, useLocale, useFocusRing, mergeProps, CalendarCellAria, AriaCalendarCellProps, DateValue } from 'react-aria';
import { isSameDay, getDayOfWeek, CalendarDate, isSameMonth } from '@internationalized/date';
...
export function CalendarCell({ state, date, currentMonth }: CalendarCellProps) {
  ...
  const { cellProps, buttonProps, isSelected, isDisabled, formattedDate, isInvalid }: CalendarCellAria = useCalendarCell({ date }, state, ref);
  ...
  const isSelectionStart =
    'highlightedRange' in state && state.highlightedRange !== null ? isSameDay(date, state.highlightedRange.start) : isSelected;

  const isSelectionEnd = 'highlightedRange' in state && state.highlightedRange !== null ? isSameDay(date, state.highlightedRange.end) : isSelected;
